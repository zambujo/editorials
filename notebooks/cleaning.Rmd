...
# TODO: export science_highlights_clean.csv
```
...
# TODO export nature_highlights_clean
```


## Combining and exporting

```{r highligts, eval=FALSE}
...
# ! hand-curated DOIs
dois <- readr::read_csv(here::here("data-raw", 
                                   "doi_curated.csv"))
dois <- dois %>%
  dplyr::rename(doi = highlight_ref_doi) %>%
  dplyr::mutate(
    doi = stringr::str_extract(doi,
                               stringr::regex(doi_regex, 
                                              ignore_case = TRUE)),
...
dois %>% 
  sample(6) %>%
  purrr::walk(cross_ref,
              polite_bow = session_bow,
              csv_path = here::here("data-raw", "doi_meta.csv"))
