parse_volumes <- function(polite_bow) {
  session <- nod(bow = polite_bow, path = "nature/volumes")
  volumes <- scrape(session) %>% html_nodes("#volume-decade-list")
...
parse_volume_issues <- function(volume, polite_bow) {
